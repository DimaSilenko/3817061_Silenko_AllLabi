...
{
	TTable<int> table(5);
	ASSERT_NO_THROW(table.SetCount(18));
	EXPECT_EQ(18, table.GetCount());
}
...
TEST(ViewingTable, can_search_elem_in_table)
{
	...
	TElem<int> elem;
	table.Put("kluch", 10);
	ASSERT_NO_THROW(elem=table.Search("kluch"));
	EXPECT_EQ("kluch", elem.GetKey());
	EXPECT_EQ(10, elem.GetData());
}
